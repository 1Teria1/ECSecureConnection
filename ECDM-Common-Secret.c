 ...
 */
int ecdh_compute_common_secret(
    EC_KEY *key,
    ...
    if (!key || !other_public || !secret_out_buffer || !secret_len_out){
        fprintf(stderr, "ECDH COMMON SECRET: передан NULL указатель\n");
        return -1;
    }
    int len = ECDH_compute_key(
        ...
        NULL);
    if (len <= 0){
        fprintf(stderr, "ECDH COMMON SECRET: Ошибка подсчёта общего секрета\n");
        return -1;
    }
    *secret_len_out = len;
    return 0;
